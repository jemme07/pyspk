...
def optimal_mass(SO, z, k, verbose=False):
    ...
    """
    if SO not in SUPPORTED_SOS:
        raise InputValidationError(f"SO must be one of {SUPPORTED_SOS}. Got: {SO!r}")
...
def get_limits(SO, z, m_halo):
    ...
    min_fb = 10 ** (
        inter_min_x0(z)
        + inter_min_x1(z) * _np.log10(m_halo)
        + inter_min_x2(z) * _np.log10(m_halo) ** 2
    )
    max_fb = 10 ** (
        inter_max_x0(z)
        + inter_max_x1(z) * _np.log10(m_halo)
        + inter_max_x2(z) * _np.log10(m_halo) ** 2
    )
...
def _akino(alpha, beta, gamma, m_halo, z, cosmo):
    """Akino et al. (2022) redshift-dependent power-law baryon fraction relation.

    Args:
        alpha: Power-law normalization parameter (log-space).
        beta: Power-law slope parameter.
    ...
    Returns:
        Baryon fraction normalized by the universal baryon fraction.
    """
    A = _np.exp(alpha) / 100
...
def _double_power_law(epsilon, alpha, beta, gamma, m_pivot, m_halo, z, cosmo):
    """Redshift-dependent double power-law baryon fraction relation.

    Args:
        epsilon: Normalization parameter.
        alpha: Power-law slope at low mass.
        beta: Power-law slope at high mass.
        gamma: Redshift dependence of the normalization.
        m_pivot: Pivot mass in M_sun units.
        m_halo: Halo mass in M_sun units.
        z: Redshift.
        cosmo: Astropy cosmology object.
...
def sup_model(
    ...
    if z < 0:
        raise InputValidationError("Incorrect redshift: z must be >= 0.")

    if z > CALIBRATED_Z_MAX:
        raise CalibrationRangeError(
            f"pyspk was calibrated up to z = {CALIBRATED_Z_MAX}. "
            f"Please specify z <= {CALIBRATED_Z_MAX}."
        )
    ...
    if k_max > CALIBRATED_K_MAX:
        raise CalibrationRangeError(
            f"pyspk was calibrated up to k_max = {CALIBRATED_K_MAX} [h/Mpc]. "
            f"Please specify k_max <= {CALIBRATED_K_MAX} [h/Mpc]."
        )
    ...
    if (fb_a is not None) or (fb_pow is not None):
        if verbose:
            msg = (
                "\033[36mUsing power-law fit for fb - M_halo at "
                f"z={z:.3f}, normalised at M_halo = {fb_pivot:.2e} [M_sun] \033[0m"
            )
            print(msg)
        if fb_a is None or fb_pow is None:
            raise InputValidationError(
                "When using a power-law, both parameters must be given: fb_a and fb_pow."
            )
        f_b = _power_law(10**best_mass, float(fb_a), float(fb_pow), float(fb_pivot))
    elif (M_halo is not None) or (fb is not None):
        if verbose:
            print(f"\033[36mUsing binned data for fb - M_halo at z={z:.3f} \033[0m")
        if M_halo is None or fb is None:
            raise InputValidationError(
                "When using binned data, both M_halo and fb arrays must be provided."
            )
        ...
        if verbose:
            print(f"\033[36mUsing double power law for fb - M_halo at z={z:.3f} \033[0m")
        if (
            epsilon is None
            or alpha is None
            or beta is None
            ...
            raise InputValidationError(
                "Using double power-law requires: epsilon, alpha, beta, gamma, m_pivot and cosmo."
            )
        f_b = _double_power_law(
            float(epsilon),
            float(alpha),
            float(beta),
            float(gamma),
        ...
        if verbose:
            msg = (
                "\033[36mUsing an Akino et al. 2022 power-law fit for fb - M_halo "
                f"at z={z:.3f}. \033[0m"
            )
            print(msg)
        if alpha is None or beta is None or gamma is None or cosmo is None:
            raise InputValidationError(
                "Using an Akino power-law requires: alpha, beta, gamma, and cosmo."
            )
        f_b = _akino(float(alpha), float(beta), float(gamma), 10**best_mass, float(z), cosmo)
    ...
    if any(out_min):
        _warnings.warn(
            (
                "\033[91mFound baryon fraction values outside fitting limits. "
                f"fb < lower_limit between {10 ** mass_out_min.min():.1e} <= M_halo [M_sun] <= "
                f"{10 ** mass_out_min.max():.1e}. "
    ...
    if errors:
        resource_name = f"stat_errors_{SO}.csv"
        try:
            package_name = __package__ or "pyspk"
            raw_text = _read_text(package_name, resource_name, encoding="utf-8")
